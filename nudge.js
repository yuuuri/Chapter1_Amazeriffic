...
function writePre(res, divClass, data) {
    ...
    res.write("<div class='" + divClass + "_div jumbotron'>\n");
    res.write("<pre>");
    res.write(escaped);
    res.write("</pre>\n");
    res.write("</div>\n");
...
function beginForm(res) {
    res.write("<form method='POST' class='form-inline' action='/push'>\n");
}

function endForm(res) {
    res.write("<input type='submit' value='Push' class='btn btn-primary'>\n");
    res.write("</form>\n");
}
...
function beginSelect(res, what) {
    res.write("<div class='" + what + "_div form-group'>\n");
